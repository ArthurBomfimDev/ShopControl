...
public class BaseValdiate<TValidateDTO> where TValidateDTO : BaseValidateDTO
{
    ...
    public static List<TValidateDTO> RemoveIgnore(this List<TValidateDTO> listValidateDTO) => (from i in listValidateDTO where !i.Ignore select i).ToList();
    #endregion
    ...
    #region Notifiction
    public static class NotifactionMessagesKey
    {
        public const string InvalidLenghtKey = "InvalidLenght";
    }
    public static class NotifcationMessage
    {
        public static string InvalidLenght(string fieldName, int minLeght, int MaxLenght) => $"O campo {fieldName} deve ter entre {minLeght} e {MaxLenght} caracters.";
    }
    ...
    {
        return HandleValidation(identifier, validateType, NotifactionMessagesKey.InvalidLenghtKey, InvalidLenght(value, minLeght, maxLenght),);
    }
